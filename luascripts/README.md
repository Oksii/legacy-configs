...
| `COLLECT_ASSIST_STATS` | `true` | Kill-assist count (`player_stats.assists`) read directly from the engine's `sess.kill_assists` counter. No Lua-side assist detection — requires a server build with the PR #3570 Lua binding; silently disabled (field omitted) on older builds, with one notification line in `stats.log` when logging is enabled (`LOGGING_ENABLED` / `STATS_API_LOG`). |
